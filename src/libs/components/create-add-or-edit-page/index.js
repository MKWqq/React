...
import { Button, Form, Input, Select, Switch,message,DatePicker,TreeSelect, } from "antd";
import Ajv from 'ajv';
...
import {isArray,isString,isReactComponent,isObject,isFunction,} from "../../../util";
...
CreateForm extends PureComponent {
  ...
  getDetailInfo = async () => {
    ...
    if (isString(detailData)) {
      const {data,ecode,} = await get(detailData);
      if (ecode === 0) {
        this.setState({
  ...
  submitData = async (val) => {
    ...
    const {ecode,message:msg,} = await post(submitUrl,params);
    if (ecode) {
      this.setState({
        loading: false,
      });
