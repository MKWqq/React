...
import {get,} from "../../../libs/api/index";
          ...
          !!items.length && (
            <Fragment>
              <div className="search-inputs">
                {
                  this.generateItems(showItems)
                }
                {
                  showHidden && this.generateItems(hiddenItems)
                }
                {
                  this.generateAction()
                }
      ...
      <Fragment>
        {
          !!hiddenItems.length && (
            <div
              ...
              key="spread"
            >
              <span className="icon icon-dot" onClick={this.toggleHiddenItems}>
                {showHidden ? '收起' : '更多'}
              </span>
            </div>
          )
      ...
      <div>
        {
          actionItems.map(z => {
            ...
            if (z.type === "add") {
              icon = z.icon || <span className={`icon icon-plus`} />;
            }
            if (z.type === "downloadTemplateFile") {
              icon = z.icon || <span className={`icon icon-xiazai`} />;
            }
            if (z.type === "importFile") {
              icon = z.icon || <span className={`icon icon-importFile`}/>;
              return (
                <div
                  key={z.title}
                  className="item-btn"
