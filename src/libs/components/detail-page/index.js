...
import { formatTimeVal, formatValue, InitImageViwer, isString } from "../../../util/index";
import {isObject,isReactComponent,isArray,} from "../../../util/index";
import {get,} from "../../api";
...
export default class DetailPage extends PureComponent {
  ...
  async getDetailInfo() {
    ...
    const {ecode,data,message:Msg,} = await get(url);
    if (ecode) {
      this.setState({
        error: Msg,
        showLoading: false,
